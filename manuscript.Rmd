...
postsampleres <- 400    # resolution of psignifit posterior sampling
...
packagelist <- c('knitr','reticulate','osfr','bookdown','rstan','coda','modeest','pals','pracma','doParallel','R.matlab','bayestestR','alphahull') # list of CRAN packages
...
allslope[subj,expt,cond,pedlevel] <- 10.3/(res$Fit[2]/C)
}
...
meanslope <- 10^apply(log10(allslope),2:4,mean)
...
for (cond in 1:12){
  ...
  MCSslope[subj,cond] <- 10.3/(res$Fit[2]/C)
...
title(main='(a) Achromatic',cex.main=2)
...
title(xlab="Pedestal contrast (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)   
title(ylab="Threshold (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
title(main='(b) Red/Green',cex.main=2)
...
title(xlab="Pedestal contrast (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
title(ylab="Threshold (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
title(main='(c) Blue/Yellow',cex.main=2)
...
modelString = "
  ...
  parameters {
    real<lower=0> p[Ndatasets];
    real<lower=0> q[Ndatasets];    
    real<lower=0> m[Ndatasets];
    real<lower=0> S[Ndatasets];  
    real<lower=0> Z[Ndatasets];
    real<lower=0> w[Ndatasets];
    real<lower=0> k[Ndatasets];
...
expdata <- data.frame(datasets,experiments,conditions,pedestals,targets,ntrials,ncorrect)


dataList = list(ncorrect = expdata$ncorrect, ntrials = expdata$ntrials, PedestalContrast = expdata$pedestals, TargetContrast = expdata$targets, c = expdata$conditions, s = expdata$datasets, Ndatasets = max(expdata$datasets), Ntotal = nrow(expdata))
  ...
  save(file=paste0('local/FullMod.RData'),list=c('stanFit'))
...
if (processdata>0){
  ...
  load(file=paste0('local/FullMod.RData'))
  
  draws <- extract(stanFit)
  
  p <- map_estimate(draws$pmu+draws$qmu)
  q <- map_estimate(draws$qmu)
  m <- map_estimate(draws$mmu)
  k <- map_estimate(draws$kmu)
  S <- map_estimate(draws$Smu)
  Z <- map_estimate(draws$Zmu)
  w <- map_estimate(draws$wmu)
  paramvect <- c(p,q,m,S,Z,w,k)
  ...
  allsamples <- array(0,dim=c(7,length(plist)))
    allsamples[1,] <- plist
    allsamples[2,] <- qlist
    allsamples[3,] <- mlist
    allsamples[4,] <- Slist
...
pdf('Figures/stanoutput.pdf',width=8,height=8)

layout(matrix(c(1,2,3,4,8,8,8,5,8,8,8,6,8,8,8,7),4,4,byrow=TRUE))
par(mar=c(3,2,1,1))

collist <- c('black','cornflowerblue','darkred')

ticklocsx <- seq(-18,24,6)
ticklabelsx <- c('1/8','1/4','1/2',1,2,4,8,16)
...
for (param in 1:7){
  ...
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(-18,24), ylim=c(0,1))
  ...
  title(xlab=paramorder[param], col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
    ...
    svals <- 20*log10(allsamples[paramvalorder[param],])
    a <- density(svals)
    a$y <- a$y/max(a$y)
    # lines(a$x,a$y,col=collist[expt])
polygon(a$x,a$y,border=NA,col='grey')
...
allpred <- array(0,dim=c(4,length(pedlist)))
  ...
  for (cond in 1:4){
    for (pedlev in 1:length(pedlist)){
      allpred[cond,pedlev] <- discriminate(paramvect,pedlevelsC[pedlev],cond)
    }
  }
...
meanslopesMCS <- 10^(apply(log10(slopematrix),c(2,3),mean))
meanslopesMCS <- round(meanslopesMCS,digits=2)
...
for (block in 1:4){
condtoplot <- cond + ((block-1)*3)
points(MCSthresh[,condtoplot],20*log10(MCSslope[,condtoplot]),pch=21,col=outlinecolours[block],bg=shadingcolours[block])
}
}    
...
```{r dipperfig, fig.cap="Dipper functions from Experiment 1, averaged across three participants. Error bars show ±1SE across participants. Note that contrast values are expressed as a percentage of the maximum displayable contrast (see Procedures for details). Curves show...", fig.align="center", echo=FALSE}
...
```{r bayesianmodel, fig.cap="Model parameters", fig.align="center", echo=FALSE}
...
We also calculated the slope of the psychometric function for each condition, converted to equivalent Weibull $\beta$ values. In the absence of a mask, the average slope was $\beta$ = `r round(10^(mean(log10(detslopes))),digits=2)`, which is typical for contrast detection tasks. Slopes became substantially steeper when the dichoptic mask matched the target in chromaticity (average $\beta$ = `r round(10^(mean(log10(c(meanslopesMCS[1,1],meanslopesMCS[2,2],meanslopesMCS[3,3])))),digits=2)`). These 'super-steep' psychometric functions for dichoptic pedestal masking have been reported previously [@Meese2006; @Baker2013], and are observed for the first time here using chromatic stimuli (see diagonal values in Figure \@ref(fig:MCSfig)h). However we did not see such markedly steep functions for any of the cross-chromaticity masking conditions (average $\beta$ = `r round(10^(mean(log10(c(meanslopesMCS[1,2],meanslopesMCS[1,3],meanslopesMCS[2,1],meanslopesMCS[2,3],meanslopesMCS[3,1],meanslopesMCS[3,2])))),digits=2)` for the off-diagonal values).
